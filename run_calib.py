...
from environment_calibration_common.clean_all import clean_analyzers, clean_logs, clean_COMPS_ID
...
from environment_calibration_common.my_func import my_func as myFunc
from environment_calibration_common.compare_to_data.run_full_comparison import *
...
site_index = 3 # TODO add site_index as an argument to parse
...
wdir = "/gpfs/projects/b1139/ipti_pmc/_mrm9534/output/"
output_dir = f"{wdir}/{exp_label}"
best_dir = f"{wdir}/{exp_label}" 

calib_coord = pd.read_csv(manifest.calibration_coordinator_path)
calib_coord = calib_coord[calib_coord.site==Site]
...
param_key=pd.read_csv("simulations/parameter_key.csv")
...
problem = Problem(workdir=f"{wdir}/{exp_label}")
